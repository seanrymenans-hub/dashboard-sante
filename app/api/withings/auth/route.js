  ...
  if (actData.status === 0 && actData.body?.activities) {
    for (const activity of actData.body.activities) {
      if (activity.steps > 0) {
        ...
        // tracker d'activité dédié (ici les pas viennent d'Apple Health relayé
        // via Withings), ce champ peut renvoyer des valeurs aberrantes (vu des
        // milliers de kcal pour une journée de marche normale). On recalcule
        // nous-mêmes avec la même formule que le Health Engine (0.04 kcal/pas),
        // pour rester cohérent avec le budget calorique affiché ailleurs.
